...
pub fn get_metadata_from_path(
    ...
    file.read_to_string(&mut buf).expect("Cannot read input as string");
    let account_metadata: AccountMetadata = serde_json::from_str(&buf[..]).expect("Account metadata json file was not well-formatted");
    ...
    for property in account_metadata.field_descriptors {
        if !map.contains_key(&property.name) {
            ...
            panic!("Property already exists in account metadata: {:#?}", property);
        }
        if !rev_map.contains_key(&(property.position as u32)) {
            match &property.typ[..] {
                ...
                "Cashflows" => rev_map.insert(property.position as u32, (property.name, Type::Cashflows)),
                _ => panic!("Invalid property type encountered in account metadata: {:#?}", property)
            };
        } else {
            panic!("Property already exists in account metadata: {:#?}", property);
        }
